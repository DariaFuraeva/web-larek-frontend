...
Методы:
- render(): Метод, который создает DOM-элемент на основе шаблона и заполняет его данными из свойств класса.
- removeFromBasket(): Метод, который удаляет элемент из корзины при нажатии на кнопку "удалить".
...
- handleDeleteCard(): void - обработчик нажатия на кнопку удаления товара из корзины
...
Методы:
- handlePaymentMethodClick(method: string) - обрабатывает клик на кнопке выбора способа оплаты и обновляет значение paymentMethod.
- handleInputAddress(event: Event) - обрабатывает ввод адреса в соответствующее поле.
- ValidateForm(): метод, который проверяет валидность формы заказа.
- submitForm(): метод, который обрабатывает кнопку "Далее".
- setValid(isValid: boolean): void - изменяет активность кнопки подтверждения.
- get form: HTMLElement - геттер для получения элемента формы.
- render(HTMLElement): HTMLElement - метод возвращает отрисованный элемент.
...
- formName: string - значение атрибута name формы.
- submitButton: HTMLButtonElement - Кнопка подтверждения
- inputEmail - введенная почта
- inputPhone - введенный номер телефона
- isValid: boolean - определяет, является ли форма заказа валидной.

Методы:
- handleInputEmail(event: Event) - обработчик для введенной почты.
- handleInputPhone(event: Event) - обработчик для введенного номера телефона.
- ValidateForm(): метод, который проверяет валидность формы заказа и обновляет значение isValid.
- submitForm(): метод, который обрабатывает кнопку "Оплатить".
- setValid(isValid: boolean): void - изменяет активность кнопки подтверждения.
- get form: HTMLElement - геттер для получения элемента формы.
- render(HTMLElement): HTMLElement - метод возвращает отрисованный элемент.
